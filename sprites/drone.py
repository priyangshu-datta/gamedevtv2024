...
class Drone(pygame.sprite.Sprite):
    ...
    def __init__(self, player: pygame.sprite.Sprite, pos, dir, groups) -> None:
        super().__init__(groups)

        if Drone.image == None:
            Drone.image = pygame.transform.rotozoom(
                pygame.image.load(
    ...
    def update(self, dt):
        ...
        if pygame.Vector2(self.rect.center).distance_to(self.player.rect.center) < 100:
            ...
            for event in events:
                if event.type == self.change_dir_event and random() > 0.5:
                    self.dir = (
                        pygame.Vector2((randint(1, w_W), randint(1, w_H - 32)))
                    ).normalize()
        ...
        if self.rect.bottom >= w_H - 32:
            self.rect.bottom = w_H - 32
        if self.rect.top <= 0:
            self.rect.top = 0
        if self.rect.left <= 0:
            self.rect.left = 0
        if self.rect.right >= w_W:
            self.rect.right = w_W
        if self.rect.bottom >= w_H - 32 or self.rect.top <= 0:
            self.dir.y *= -1
